...
async def persist_events(session: AsyncSession, events: list[NormalizedEvent]) -> tuple[int, int]:
    ...
    for event in events:
        ...
        if existing:
            ...
            continue
        session.add(EventModel(id=event.event_id, event_type=event.event_type, title=event.description[:500], description=event.description, event_timestamp=event.timestamp, confidence=event.confidence, status=event.status, source_count=event.source_count, metadata_json=event.metadata or {"source_urls": list(event.source_urls)}))
        for source_id in event.source_ids:
            url = next((u for u in event.source_urls if u), "")
            session.add(EventSourceModel(event_id=event.event_id, source_id=source_id, source_url=url, source_title=event.description[:500], raw_json=event.metadata or {}))
        new_count += 1
    await session.flush()
...
async def create_snapshot(session: AsyncSession, simulation_id: str, tick: int, changes: dict) -> None:
    session.add(WorldSnapshotModel(tick=tick, simulation_id=simulation_id, model_version="world-state-v1", dataset_version="live", random_seed=0, state_json={"actor_changes": changes}))
    session.add(SimulationTickModel(simulation_id=simulation_id, tick=tick, event_ids=[], state_changes=changes))
