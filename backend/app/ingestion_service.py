...
def validate_gdelt(rows):
    ...
    for row in rows:
        ...
        if not title or not row.get('source_url'):
            continue
        events.append(RawEvent(title=title, description=str(row.get('description') or title),
            event_type='news', source_id='gdelt', source_url=row.get('source_url',''),
            confidence=.55, timestamp=_parse_dt(row.get('timestamp'))))
    return events
...
def validate_world_bank(rows):
    ...
    for row in rows:
        ...
        events.append(RawEvent(title=title, description=description, event_type='economic_indicator',
            source_id='worldbank', confidence=.98, actors=(str(row['country']).lower(),),
...
async def sync_sources(session, query='geopolitics', max_records=25):
    gdelt_rows, wb_rows = await fetch_gdelt(query, max_records), await fetch_world_bank()
    raw = validate_gdelt(gdelt_rows) + validate_world_bank(wb_rows)
    normalized = normalize(raw)
    new, duplicate = await persist_events(session, normalized)
    await session.commit()
    return {'fetched': len(raw), 'normalized': len(normalized), 'new': new, 'duplicates': duplicate,
