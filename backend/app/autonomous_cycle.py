...
async def run_cycle(query: str = "geopolitics", max_records: int = 25) -> dict:
    ...
    return {"simulation_id": run_id, "status": "completed", "started_at": started.isoformat(), "finished_at": datetime.now(timezone.utc).isoformat(), "raw_events": len(raw), "news_events": len(raw_news), "indicator_events": len(raw_indicators), "normalized_events": len(normalized), "new_events": new_count, "duplicates": duplicate_count, "state_changes": changes, "events": [{"event_id": e.event_id, "timestamp": e.timestamp.isoformat(), "actors": list(e.actors), "event_type": e.event_type, "description": e.description, "source_ids": list(e.source_ids), "source_count": e.source_count, "confidence": e.confidence, "status": e.status, "source_urls": list(e.source_urls)} for e in normalized]}
