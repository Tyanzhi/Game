"""Actor Decision Engine: deterministic, explainable strategic decisions under incomplete information."""
...
def _utility(action: str, actor: ActorModel, threat: float, pressure: float, economic_signal: float, relationship: float) -> tuple[float, tuple[str, ...]]:
    benefits = {"observe": .25 + actor.information_quality * .25, "diplomatic_outreach": actor.diplomatic_capacity * .42 + max(0, -relationship) * .30, "economic_adjustment": actor.economic_capacity * .38 + economic_signal * .25, "defensive_posture": actor.security_capacity * .40 + threat * .42, "public_statement": actor.diplomatic_capacity * .18 + pressure * .18}
    costs = {"observe": threat * .34, "diplomatic_outreach": pressure * .12, "economic_adjustment": .18 + actor.domestic_pressure * .18, "defensive_posture": .22 + pressure * .16, "public_statement": .10 + threat * .08}
    rationale = []
...
async def decide(session: AsyncSession, actor_id: str, simulation_id: str | None = None, event_limit: int = 30) -> dict:
    ...
    for event in events:
        ...
        text = (event.description or "").lower()
        if actor_id in event_actors or actor_id in text:
    ...
    options.sort(key=lambda o: (-o.expected_utility, o.action))
    selected = options[0]
    confidence = _clamp(0.45 + abs(selected.expected_utility - options[1].expected_utility) * 1.8 + actor.information_quality * .2)
    decision_id = "dec-" + sha256(f"{actor_id}|{simulation_id}|{datetime.now(timezone.utc).isoformat()}".encode()).hexdigest()[:24]
    ...
    decision = DecisionModel(id=decision_id, actor_id=actor_id, simulation_id=simulation_id, selected_action=selected.action, confidence=confidence, situation=f"tick={datetime.now(timezone.utc).isoformat()}; relevant_events={len(relevant)}", reasoning_factors=factors, options=option_json, information_state=information_state)
    session.add(decision)
    action_id = "act-" + sha256(f"{decision_id}|{selected.action}".encode()).hexdigest()[:24]
    effects = {"action": selected.action, "expected_utility": selected.expected_utility, "risk": selected.risk, "confidence": confidence}
    session.add(ActionModel(id=action_id, decision_id=decision_id, actor_id=actor_id, action_type=selected.action, status="executed", effects=effects))
    await session.flush()
    return {"decision_id": decision_id, "actor_id": actor_id, "selected_action": selected.action, "confidence": confidence, "reasoning_factors": factors, "information_state": information_state, "options": option_json, "action_id": action_id, "effects": effects}
